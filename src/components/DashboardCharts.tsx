...
import { getTaskBreakdown, getProductivityData, type ProductivityPeriod } from '@/lib/dashboardCharts'
...
export function ProductivityChart({ tasks }: ProductivityChartProps) {
  ...
  const data = getProductivityData(tasks, period)
  ...
  return (
    <div className="rounded-lg border border-slate-600 bg-slate-800/50 p-4">
      ...
      <div className="h-64">
        {data.length === 0 ? (
          <div className="flex h-full items-center justify-center text-slate-400">No completed tasks in this period</div>
        ) : (
          <ResponsiveContainer width="100%" height="100%">
